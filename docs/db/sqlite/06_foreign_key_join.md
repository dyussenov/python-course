...
Также мы можем добваить оператор `ON DELETE`, который устанавливает поведение при удалении записи из родительской таблицы:

- `ON DELETE CASCADE`: При удалении строки из родительской таблицы, все связанные строки из дочерней таблицы также будут удалены автоматически.

- `ON DELETE SET NULL`: При удалении строки из родительской таблицы, значение в столбце дочерней таблицы, ссылавшегося на эту строку, будет установлено в NULL. 

- `ON DELETE SET DEFAULT`: Устанавливает значение столбца дочерней таблицы, ссылавшегося на удаленную строку в родительской таблице, равным значению столбца "по умолчанию". Для установки значения по умолчанию с соответствующем поле нужно указать оператор `DEFAULT`.
