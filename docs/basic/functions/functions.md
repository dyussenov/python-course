...
Чистые функции (pure functions) – это функции, которые не имеют побочных эффектов (вывод в консоль) и всегда возвращают одинаковый результат для одних и тех же аргументов. В чистых функциях нет изменяемых состояний и они не зависят от внешних переменных, поэтому они легко тестируются и предсказуемы в использовании.
